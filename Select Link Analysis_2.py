...
## Access links - from centroid therefore OUT
    for i in range(0,len(centroid_dict[c]['access'])):
        ...
        updated_network_OUT = scenario.get_network()
        ...
        for link in updated_network_OUT.links():
        ...
        data.clear()
        reset_selectlink(updated_network_OUT)

        if updated_network_OUT.link(aclink.i_node,aclink.j_node)['@select'] == 1:
    ...
    for i in range(0,len(centroid_dict[c]['egress'])):
            ...
            for link in network.links():
                if network.link(link.i_node,link.j_node)['@select'] == 1:
                    print("@select=1: {3} - link {0}-{1}".format(link.i_node,link.j_node,c))
            ...
            # After running the assignment, the scenario will have updated @select_volumes automatically saved
            # We just need to 'refresh' the network object to fetch these updated volumes
            updated_network_IN = scenario.get_network()
            ...
            for link in updated_network_IN.links():
                f.write("{0};{1}\n".format(link.id,link["@select_volume"]))
                link_id = str(link.i_node)+"-"+str(link.j_node)
                if link_id in data.keys():
                    data[link_id].append(link["@select_volume"])
            ...
            data.clear()
            reset_selectlink(updated_network_IN)

            if updated_network.link(eglink.i_node,eglink.j_node)['@select'] == 1:
                print("Did not reset links to 0 for ",updated_network_OUT.link(eglink.i_node,eglink.j_node))
            else:
                print("all links for @select is set to 0")
