...
# Set link
# network.link('11145','11148')['@select'] = 1
network.link('10030','10063')['@select'] = 1


# Get link id
for link in network.links():
    if network.link(link.i_node,link.j_node)['@select'] == 1:
        print(f'Select link anaysis for link: {link.id}')
...
def setatt(type,attribute):
    selectatt = scenario.get_attribute_values(type, [attribute])
    scenario.set_attribute_values(type, [attribute], selectatt)
...
for c in centroid_dict:
    ...
    for i in range(0,len(centroid_dict[c]['access'])):
        ...
        print(scenario.has_traffic_results)
        # Store results
        print(network.link('10030','10063')['@select_volume'])
